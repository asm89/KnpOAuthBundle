...
namespace Knp\Bundle\OAuthBundle\Controller;
...
class SecurityController extends ContainerAware
{
    ...
    {
        if (!$this->container->getParameter('knp_oauth.firewall_name')) {
            throw new \RuntimeException('A firewall_name should be configured in order to use the default login action.');
        }

        $ownerMap = $this->container->get('knp_oauth.resource_ownermap.'.$this->container->getParameter('knp_oauth.firewall_name'));
        ...
        return $this->container->get('templating')->renderResponse('KnpOAuthBundle:Security:login.html.twig', array(
            'error'         => $error,
